...
func _init(from = {}, 
	...
	self._stored_pack = KeyValuePack.pack_to_pack(from, PackType.DICTIONARY_PACK, init_redundant)
...
func has_pack(pack) -> bool:
	return KeyValuePack.pack_has_pack(self, pack)

# (read-only, container producing)
## Returns a new dictionary pack converted from this pack.
func to_dict() -> Dictionary:
	return KeyValuePack.pack_to_pack(self, PackType.DICTIONARY_PACK, self._redundant)

# (read-only, container producing)
## Returns a new array pack converted from this pack.
func to_arr() -> Array:
	return KeyValuePack.pack_to_pack(self, PackType.ARRAY_PACK, self._redundant)
...
func add_pack(pack) -> void:
	return KeyValuePack.pack_add_pack(self, pack, self._redundant)
...
func _iter_continue() -> bool:
	while self._iter_x < self._iter_keys.size():
		...
		self._iter_keystate = KeyValuePack.pack_get_keystate(self, self._iter_k)
...
# (static)
## Returns the type in [enum KeyValuePack.PackType] of given [param pack].
...
# (static)
## Returns the state in [enum KeyValuePack.KeyState] of given [param key] in given [param pack].
	...
	match pack_get_packtype(pack):
		...
		PackType.DICTIONARY_PACK:
			return KeyState.NONE if not pack.has(key) else \
					...
					KeyState.HAS_ARRAY
		PackType.KVPACK:
			return pack_get_keystate(pack._stored_pack, key)
		_: 
...
# (static)
## Returns true if [param pack1] has all pairs given by [param pack2].
static func pack_has_pack(pack1, pack2) -> bool:
	return _pack_iterator(pack2, func(kvpair): return _pack_has_kvpair(pack1, kvpair))

# (static, private, container writing)
...
## In [param pack1], add pairs given by [param pack2].
## [br]When [param redundant] is false, ignore any pair that already exists.
...
# (static, container writing)
## In [param pack1], delete pairs given by [param pack2].
...
## Returns a new pack in given [param packtype] converted from given [param pack].
## [br]See [enum KeyValuePack.PackType] for [param packtype].
## [br]When [param redundant] is [code]false[/code], 
## ignore any redundant pair.
static func pack_to_pack(pack, packtype := PackType.KVPACK, redundant := false):
	var new_pack
	match packtype:
		PackType.KVPAIR:
			new_pack = [null, null]
	...
	pack_add_pack(new_pack, pack, redundant)
	return new_pack
