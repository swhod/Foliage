...
## {"odd": [1, 3, 5], "even": 2}
## [/codeblock]
## [br]Both forms are accepted by this class. Besides, [b]a 
## single pair is also supported[/b]. All valid forms are
## listed in [enum KeyValuePack.PackType].
## [br]For the usage of going through all elements,
## this class has its own iterator. To use it, simply write
## [code]in[/code] with [code]for[/code]:
## [codeblock]
...
# (read-only)
## Returns the state in [enum KeyValuePack.KeyState] of given [param key]. 
func get_keystate(key) -> int:
	return KeyValuePack._pack_get_keystate(self, key)
...
# (read-only)
## Returns a new pack in [param packtype] converted from this pack.
## [br]See [enum KeyValuePack.PackType] for [param packtype].
## [br]When [param redundant] is [code]false[/code], 
## ignore any redundant pair.
func to_pack(packtype := PackType.KVPACK, redundant := self._redundant):
	return KeyValuePack._pack_to_pack(self, packtype, redundant)
...
## [br]When [param redundant] is [code]false[/code], 
## ignore any pair if it already exists.
func add_pack(pack, redundant := self._redundant) -> void:
	return KeyValuePack._pack_add_pack(self, pack, redundant)
...
func _iter_get(_arg) -> Array:
	...
	self._iter_return[VALUE] = self._output_map.call(self._iter_return[VALUE])
...
# (static, private)
# Returns the state of given key in given pack.
...
# (static, private)
# Returns true if the first pack has all pairs given by the second pack.
...
# (static, private, container writing)
# In the first pack, add pairs given by the second pack.
